  ...
  return <View style={{ borderWidth: 1, borderColor: current || pinned ? theme.colors.accent : theme.colors.border, borderRadius: 10, padding: 12, gap: 10, backgroundColor: theme.colors.surface1 }}>
    <View style={{ gap: 3 }}>
      <View style={{ flexDirection: 'row', alignItems: 'center', flexWrap: 'wrap', gap: 8 }}>
        ...
        {pinned ? <Text style={{ color: theme.colors.accent, fontSize: 12 }}>{ui('Header', '顶栏')}</Text> : null}
      </View>
    ...
    {usage.status !== 'available' ? <Text style={{ color: theme.colors.foregroundMuted, fontSize: 13 }}>{ui('Unavailable · Check this provider’s sign-in status', '暂不可用 · 请检查该供应商的登录状态')}</Text> : <>
      ...
      {usage.balances?.filter(balance => balanceRemaining(balance) !== null).map(balance => <Meter key={balance.id} label={balance.label} value={balancePercent(balance)} text={ui(`${formatBalance(balance)} remaining`, `剩余 ${formatBalance(balance)}`)} resetsAt={balance.resetsAt} theme={theme} />)}
  ...
  const body = <View style={{ gap: popover ? 10 : 16, width: '100%', maxWidth: 780, alignSelf: 'center' }}>
    <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
      <View style={{ gap: 2, flexShrink: 1 }}>
        <Text style={{ color: theme.colors.foreground, fontSize: popover ? 15 : 25, fontWeight: '700' }}>{ui('Quota on this host', '本机额度')}</Text>
        <Text style={{ color: theme.colors.foregroundMuted, fontSize: popover ? 11 : 12 }}>{localizeHostLabel(host.label)} · {dataAge(result.data?.fetchedAt)}</Text>
      </View>
      <Pressable accessibilityRole="button" accessibilityLabel={ui('Refresh quota', '刷新额度')} disabled={result.isFetching} onPress={() => { void result.refetch(); }} style={{ borderWidth: 1, borderColor: theme.colors.border, borderRadius: 7, minHeight: layout.compact ? 44 : 30, justifyContent: 'center', paddingHorizontal: 10, paddingVertical: 5, opacity: result.isFetching ? 0.55 : 1 }}>
        <Text style={{ color: theme.colors.foreground, fontSize: 12 }}>{result.isFetching ? ui('Updating…', '更新中…') : ui('Refresh', '刷新')}</Text>
      </Pressable>
    </View>
