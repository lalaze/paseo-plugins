...
import { formatTokens, sourceNames, type ConsumptionReport, type Tokens } from '../shared/consumption';
  ...
  return <View style={{ borderTopWidth: 1, borderTopColor: theme.colors.border, gap: 4 }}>
    <Pressable accessibilityRole="button" accessibilityLabel="数据来源与统计说明" aria-expanded={expanded} accessibilityState={{ expanded }} onPress={() => setExpanded(value => !value)} style={({ pressed }) => ({ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', gap: 8, minHeight: 44, opacity: pressed ? 0.65 : 1 })}>
      ...
      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, flexShrink: 1 }}>
        <View style={{ width: 5, height: 5, borderRadius: 3, backgroundColor: issues.length ? theme.colors.statusWarning : loading || !sources.length ? theme.colors.foregroundMuted : theme.colors.statusSuccess }} />
    ...
    {report?.unsupportedProviders?.map(provider => <Text key={provider.id} style={{ fontSize: 11, lineHeight: 17, color: theme.colors.foregroundMuted }}>{provider.label} · 暂不支持消耗统计</Text>)}
    {expanded ? <View style={{ gap: 10, paddingTop: 8, paddingBottom: 4 }}>
      {sources.map(source => <View key={source.source} style={{ gap: 4 }}>
        <Text style={{ fontSize: 11, color: source.status === 'error' || source.status === 'partial' ? theme.colors.statusWarning : theme.colors.foregroundMuted }}>{sourceNames[source.source]} · {{ loading: '读取中', ready: '已读取', empty: '该时段无记录', partial: '记录不完整', error: '读取失败' }[source.status]}{source.updatedAt ? ` · ${dataAge(source.updatedAt)}` : ''}</Text>
      ...
      <Text style={{ color: theme.colors.foregroundMuted, fontSize: 11, lineHeight: 18 }}>跟随本机 Providers 开关，统计已启用来源在此主机保留的记录，含终端直接运行的会话。已删除或未记录的调用无法补回；Grok 等来源可能在轮次结束后才计入。</Text>
    </View> : null}
  </View>;
}
