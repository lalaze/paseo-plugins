...
export function formatBalance(balance: UsageBalance): string {
  ...
  const unit = { credits: '积分', requests: '次', tokens: 'tokens' }[balance.unit];
...
export function summary(usage?: Usage, model?: string | null) {
  if (!usage || usage.status !== 'available') return { label: '额度暂不可用', tone: 'unknown' as Tone, remainingPct: null, detail: '当前供应商尚无可用额度数据' };
  ...
  if (metrics.length) {
    ...
    const qualifier = metrics.length > 1 ? '最低剩余' : '剩余';
    return { label: `${qualifier} ${formatPercent(lowest.value)}`, tone: tone(lowest.value), remainingPct: lowest.value, detail: `${usage.displayName} · ${lowest.label} · 剩余 ${formatPercent(lowest.value)}` };
  }
  const balance = usage.balances?.find(item => balanceRemaining(item) !== null);
  if (balance) return { label: `剩余 ${formatBalance(balance)}`, tone: balanceRemaining(balance) === 0 ? 'danger' as Tone : 'ok' as Tone, remainingPct: null, detail: `${usage.displayName} · ${balance.label}` };
  return { label: '额度未知', tone: 'unknown' as Tone, remainingPct: null, detail: `${usage.displayName} 未返回可计算的剩余额度` };
}
...
function pinnedHeader(usage: Usage) {
  ...
  if (usage.status !== 'available') {
    return {
      ...
      detail: `顶栏固定显示 ${usage.displayName}，当前暂无可用额度。点击可改回最低剩余或查看全部额度`,
    };
  }
  const brief = summary(usage);
  return {
    label: brief.remainingPct !== null ? `${name} 余${formatPercent(brief.remainingPct)}` : `${name} ${brief.label}`,
    tone: brief.tone,
    detail: `顶栏固定显示 ${brief.detail}。点击可改回最低剩余或查看全部额度`,
  };
}
...
export function headerSummary(providers: Usage[], selectedId?: string | null) {
  ...
  if (!lowest) return { label: '额度 · 查看明细', tone: 'unknown' as Tone, detail: '尚无可比较的百分比额度，点击查看余额与明细' };
  const name = providerShortName(lowest.usage);
  return {
    label: `${name} 余${formatPercent(lowest.brief.remainingPct!)}`,
    tone: lowest.brief.tone,
    detail: selectedId
      ? `已选择的供应商暂无数据，当前显示最低剩余：${lowest.brief.detail}。点击查看全部额度`
      : `所有可用额度中的最低剩余：${lowest.brief.detail}。点击查看全部额度`,
  };
}
