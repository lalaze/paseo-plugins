...
- 对话页提供「翻译输入」快捷入口；卡片内可直接输入或粘贴文字，并支持 `Ctrl/Cmd + Enter` 快速翻译。
...
翻译结果卡片右上角的「设置」也会打开同一配置页。修改后直接保存，无需 reload。卡片会自动翻译划选内容；也可以点击对话页右下方的「翻译输入」打开空输入框。编辑后点击「翻译」或按 `Ctrl/Cmd + Enter` 翻译。
