...
- 团队配置保存在当前 Paseo 主机，后续任务可复用。可在 AI 协作面板创建任务，也可在已有项目工作区的 Agent 输入框发送 `/director 任务描述`，使用当前目录和已保存团队下发并打开进度页。命令不带参数时只打开面板，不自动继承当前聊天历史或接管当前 Agent。Paseo 0.8 的「新建工作区」页面尚无工作区上下文，不加载插件命令；要直接创建协作任务，使用侧栏 AI 协作入口。
...
| 审核 AI | 检查实际成果与验收证据，决定通过、返工或受阻 | 完整成果的统一审核结论 |
...
允许多个执行 AI 配置，但保持串行，同一时刻仅有一个 AI 写入该任务工作区。前置任务执行完成即可派发后续依赖任务，全部任务完成后，审核 AI 才对代码静止时的完整成果统一审核。
...
- 未指定 `reviewerProfileId` 时，审核回到原 `directorAgentId`。显式指定时，统一审核及复审使用独立 `reviewerAgentId`，包括模型与设计者相同的情况。缺失审核会话只按审核配置重建，不回退到设计模型。新配置不修改已有任务快照。
...
每个执行者提交结果后，插件确认其当前轮次已结束，再派发下一项任务；全部执行完成后，运行指定检查并保存可重复读取的完整成果版本，再请求统一审核。Git 项目使用 base/head commit 与未提交改动快照标识成果，避免审核对象在过程中变化。
...
默认由审核 AI 读取实际成果、自行选择验证方式并作出审核决定，用户只需安排设计、执行和审核角色，无需预先填写测试命令。插件始终保存成果快照并检查逐项审核依据。用户可选地指定额外检查命令，插件运行这些命令并保存原始结果；已指定检查失败时不得由 AI 的通过结论覆盖。未指定检查不标记为测试通过。统一审核须覆盖总纲及所有子任务的验收标准；通过后所有任务才标记为已通过，并进入用户验收。成果版本改变时，相应验收失效。
...
主流程状态：`planning → executing → final_review → awaiting_acceptance → completed`（`reviewing` 仅用于恢复旧版本已派发的单项审核）。控制状态有 `running`、`paused`、`waiting_permission`、`needs_attention`、`canceling`、`canceled`。
