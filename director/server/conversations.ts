...
import { SettingsSchema, type ControlAction, type Run } from "../shared/schema";
...
export class Conversations {
  ...
  async open(input: { requestId: string; workspaceId: string; goal?: string; fresh?: boolean; conversationId?: string }) {
  ...
  private async user(c: Conversation, id?: string) {
    ...
    const automatic = new Set([...c.notices.map(n => n.id), ...(c.runId ? this.store.get(c.runId).operations.map(o => o.id) : [])]);
    const latest = items.filter(i => i.type === "user_message" && messageId(i) && !automatic.has(messageId(i)!)).at(-1);
  ...
  async status(id: string) {
    return this.locked(async () => {
      ...
      if (c.error?.includes("MCP")) c.error = undefined;
      this.store.saveConversation(c);
  ...
  async resync(id: string) {
    return this.locked(async () => {
      ...
      if (c.agentId && (await this.gateway.inspect(c.agentId, "recover-main")).status === "missing") {
  ...
  async tick() {
    return this.locked(async () => {
      ...
      for (const existing of this.store.conversations()) {
        let c = existing;
        try {
          if (!c.linksReady || !c.runId || c.state !== "ready" || (!c.initialDelivered && c.initialGoal && !c.runId)) await this.ensure(c);
          if (c.agentId && !c.toolsConnectedAt && !c.runId) {
            const state = await this.gateway.inspect(c.agentId, "tool-handshake");
            const { latest } = await this.user(c);
            if (latest && ["idle", "error"].includes(state.status)) {
              c.error = "主 Agent 尚未成功连接协作 MCP 工具。请检查所选接入是否支持 HTTP MCP；确认后在主对话重试，尚未启动后台任务。";
              this.store.saveConversation(c);
            }
          }
