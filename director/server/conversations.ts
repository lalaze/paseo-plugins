...
export interface ConversationGateway {
  ...
  conversationHistory(agentId: string, stop?: (items: AgentTimelineItem[]) => boolean): Promise<AgentTimelineItem[]>;
  appendConversationLink(agentId: string, conversationId: string): Promise<void>;
  inspect(agentId: string, messageId: string, since?: number): Promise<AgentSnapshot>;
...
export class Conversations {
  ...
  /** One chain per conversation, so a slow poll of one chat never delays another chat's tool calls. */
  ...
  private async ensureLocked(id: string) {
    return this.locked(id, async () => { const c = this.store.conversation(id); await this.ensure(c); return this.summary(c.id); });
  }
  async close() { this.closed = true; await Promise.allSettled([...this.locks.values()]); }
  ...
  private async takeover(input: { requestId: string; workspaceId: string; agentId: string; goal?: string }) {
    ...
    const id = await this.locked("create", async () => {
      ...
      if (!c.takeover) { c.takeover = { messages: [] }; c.linksReady = false; this.store.saveConversation(c); }
      const commandId = `chat-command:${digest(input.requestId)}`;
      if (!c.takeover.messages.some(m => m.id === commandId) && (input.goal?.trim() || !c.takeover.messages.length)) {
        c.takeover.messages.push({ id: commandId, text: input.goal?.trim() || "用户已在当前对话启用协作。请确认工具连接并简短告知已就绪。此消息只启用协作，不授权启动任务，不根据历史消息自动实施。", automatic: !input.goal?.trim(), state: "pending", createdAt: Date.now() });
        this.store.saveConversation(c);
      }
      return c.id;
    });
  ...
  private async ensure(c: Conversation) {
    try {
      ...
      if (c.state !== "ready" || c.error !== undefined) { c.state = "ready"; c.error = undefined; this.store.saveConversation(c); }
      if (!c.linksReady) {
      ...
      if (c.takeover) {
        for (const message of c.takeover.messages.filter(m => m.state !== "sent")) {
          const state = await this.gateway.inspect(c.agentId, message.id, message.createdAt);
      ...
      if (!c.initialDelivered && c.initialGoal && !c.runId) {
        ...
        const state = await this.gateway.inspect(c.agentId, id, c.createdAt);
  ...
  private async user(c: Conversation, id?: string, need: (items: AgentTimelineItem[]) => boolean = () => true) {
    ...
    const candidate = (i: AgentTimelineItem) => i.type === "user_message" && !!messageId(i) && !automatic.has(messageId(i)!);
    // Paging back from the tail, the first candidate is the latest one. It lies
    // after the takeover boundary whenever that command is known to be delivered.
    const delivered = !firstCommand || c.takeover!.messages[0].state === "sent";
    const items = await this.gateway.conversationHistory(c.agentId, items => need(items)
      && (delivered ? items.some(i => candidate(i) || messageId(i) === firstCommand) : items.some(i => messageId(i) === firstCommand)));
    const boundary = firstCommand ? items.findIndex(i => messageId(i) === firstCommand) : 0;
    const eligible = boundary < 0 ? [] : items.slice(boundary);
    const latest = eligible.filter(candidate).at(-1);
  ...
  async control(id: string, input: { action: ControlAction; sourceMessageId: string; confirmationKey?: string; goal?: string; feedback?: string }) {
    return this.locked(id, async () => {
      ...
      // Approvals are validated against the notice they answer, so page back until it is present too.
      const anchor = c.confirmation?.noticeId;
      const { items } = await this.user(c, input.sourceMessageId, items => !anchor || items.some(i => messageId(i) === anchor));
  ...
  async migrate() {
    return this.locked("create", async () => {
      for (const run of this.store.legacyRuns()) {
  ...
  async resync(id: string) {
    return this.locked(id, async () => {
      ...
      if (c.takeover) {
        if (!c.agentId || (await this.gateway.inspect(c.agentId, "recover-main", Date.now())).status === "missing") throw new Error("原对话已不可用，无法原地接管；请从历史恢复此对话");
        c.linksReady = false;
      ...
      if (c.takeover && !c.takeover.messages.some(m => m.state !== "sent")) {
        c.takeover.messages.push({ id: `chat-command:resync:${c.id}:${c.takeover.messages.length}`, text: "用户已在当前对话启用协作。请重新查询状态，确认工具可用；这不是新的实施任务。", automatic: true, state: "pending", createdAt: Date.now() });
      }
      if (!c.takeover && c.agentId && (await this.gateway.inspect(c.agentId, "recover-main", Date.now())).status === "missing") {
  ...
  async tick() {
    ...
    for (const { id } of this.store.conversations()) {
      if (this.closed) return;
      await this.locked(id, async () => {
        ...
        try {
          ...
          if (c.takeover?.messages.some(m => m.state !== "sent")) return;
          if (c.agentId && !c.toolsConnectedAt && !c.runId) {
            const state = await this.gateway.inspect(c.agentId, "tool-handshake", Date.now());
            const { latest } = await this.user(c);
          ...
          if (key !== c.noticeKey) {
            ...
            c.notices.push({ id, key, text, state: "pending", createdAt: Date.now() }); c.noticeKey = key;
          ...
          if (op?.agentId === c.agentId && !["done", "abandoned"].includes(op.state)) return;
          for (const notice of c.notices.filter(n => n.state !== "sent")) {
            const state = await this.gateway.inspect(c.agentId, notice.id, notice.createdAt);
