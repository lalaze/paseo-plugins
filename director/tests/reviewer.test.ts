...
test("selected reviewer owns task reviews, rework and final review; user feedback returns to the original designer", async t => {
  ...
  const auditor = await h.until("review");
  ...
  assert.deepEqual(evidence.plan, plan); assert.deepEqual(evidence.result, result); assert.equal(evidence.evidence.id, "artifact-v1");
  ...
  await assert.rejects(h.engine.submit(h.id, "director", auditor.id, review()), /角色无权/);
  await assert.rejects(h.engine.submit(h.id, plan.tasks[0].id, auditor.id, review()), /角色无权/);
  await h.complete(review(false, "changes_requested")); await h.restart();
  const redo = await h.until("execute"); assert.equal(redo.agentId, worker.agentId); assert.match(redo.prompt, /增加空值处理/);
  await h.complete(result); assert.equal((await h.until("review")).agentId, auditor.agentId); await h.complete(review());
  ...
  assert.equal((await h.until("review")).agentId, auditor.agentId); await h.complete(review());
...
test("explicitly choosing the same model still separates designer, worker and reviewer sessions", async t => {
  ...
  const worker = await h.until("execute"); await h.complete(result);
  const auditor = await h.until("review"); await h.complete(review());
  assert.equal((await h.until("final")).agentId, auditor.agentId);
...
test("old settings use the original director even if saved defaults change during the run", async t => {
  ...
  assert.equal((await h.until("review")).agentId, design.agentId); await h.complete(review());
...
test("reviewer permissions and missing-session recovery never fall back to the designer", async t => {
  ...
  const auditor = await h.until("review"), sent = h.agents.sent.length;
  ...
  const replacement = await h.until("review"); assert.equal(replacement.profileId, "audit");
  ...
  await h.complete(review()); assert.equal((await h.until("final")).agentId, replacement.agentId);
});
...
test("a stale artifact cannot be approved by the separate reviewer", async t => {
  ...
  await h.until("review"); h.repository.version = "artifact-v2"; await h.complete(review());
  assert.equal(h.run().control, "needs_attention"); assert.match(h.run().message, /版本/);
  assert.equal(h.run().tasks[0].status, "reviewing");
});
