...
test("MCP isolates reviewer tools and credentials from designer and legacy task IDs", async t => {
  ...
  const audit = await h.until("review");
  await assert.rejects(h.engine.submit(h.id, "reviewer", audit.id, review()), /角色无权/);
  await assert.rejects(h.engine.submit(h.id, "director", audit.id, review()), /角色无权/);
  const accepted = await reviewer.callTool({ name: "submit_review", arguments: { operationId: audit.id, payload: review() } });
  assert.notEqual(accepted.isError, true);
  await h.complete(review()); const final = await h.until("final"); assert.equal(final.agentId, audit.agentId);
  const stale = await reviewer.callTool({ name: "submit_review", arguments: { operationId: audit.id, payload: review() } });
  assert.equal(stale.isError, true);
  assert.notEqual((await reviewer.callTool({ name: "submit_review", arguments: { operationId: final.id, payload: review(true) } })).isError, true);
