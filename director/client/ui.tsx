...
export function ErrorText({ error, theme }: { error: unknown; theme: Theme }) {
  return error ? <Text accessibilityRole="alert" selectable style={{ color: theme.colors.statusDanger, lineHeight: 21 }}>{localizeDirectorMessage(error instanceof Error ? error.message : String(error))}</Text> : null;
}
