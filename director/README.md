...
2. 在已有项目工作区选择「新建协作对话」，或者发送 `/director 任务描述`。空的 `/director` 恢复已有协作会话，没有时创建空白主对话。新主对话由宿主加入当前工作区标签栏，点击标签开始交流。
...
首次未配置时会打开协作设置并保留命令中的目标，保存后继续创建主对话。若主 Agent 没有成功调用协作工具，可在主对话询问状态并检查 MCP；原生子会话数量不代表后台任务已启动。连接错误保留状态，可点击“重新同步到主对话”。

不再注册插件工作区面板，也不显示“主对话已准备好”中转页。Paseo 0.8 的命令接口不提供原生标签切换能力，创建完成后请点击主 Agent 标签；已经手动关闭的会话可从工作区会话历史重新打开。
