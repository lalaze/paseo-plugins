...
4. 通过 Paseo 原生的 `subagent` 入口查看和打开关联子会话；插件不再添加重复的胶囊或列表。暂停、继续、停止协作请在主对话中提出，协作设置按钮保留在工作区顶栏。
...
首次未配置时会打开设置并保留命令中的目标。若主 Agent 没有成功调用协作工具，可在主对话询问状态并检查 MCP；原生子会话数量不代表后台任务已启动。连接错误保留状态，可点击“重新同步到主对话”。
