...
export function FileTree(props: TreeProps) {
  return <div role="tree" aria-label={ui('Workspace file tree', '工作区文件树')} style={{ padding: '4px 0' }}>
...
function Directory(props: TreeProps & { path: string; name: string; root?: boolean }) {
  ...
  useEffect(() => {
    ...
    list({ workspaceId: props.workspaceId, path }).then(result => { if (current) setEntries(result); })
      .catch(error => { if (current) setError(localizeFileError(error)); })
  ...
  return <div role="treeitem" aria-label={name} aria-expanded={expanded} aria-selected={selected === path}>
    <button type="button" data-folder-path={path} title={path || props.rootPath} aria-label={`${expanded ? ui('Collapse', '折叠') : ui('Expand', '展开')} ${name}`} disabled={busy}
      ...
      {targeted && <span style={{ fontSize: 11, flexShrink: 0 }}>{ui('Upload here', '上传到这里')}</span>}
    </button>
    {expanded && <div role="group" style={{ marginLeft: 13, paddingLeft: 5, borderLeft: `1px solid ${colors.border}` }}>
      {loading && <div style={{ padding: '5px 8px', color: colors.foregroundMuted }}>{ui('Loading…', '正在读取…')}</div>}
      {error && <div role="alert" style={{ padding: '5px 8px', color: colors.statusDanger, overflowWrap: 'anywhere' }}>{error} <button onClick={() => setRetry(value => value + 1)}>{ui('Retry', '重试')}</button></div>}
      {!loading && !error && entries.length === 0 && <div style={{ padding: '5px 8px', color: colors.foregroundMuted }}>{ui('Empty folder', '空文件夹')}</div>}
      {!error && entries.map(entry => entry.directory
        ...
        : <div key={entry.path} role="treeitem" aria-label={entry.name} data-file-path={entry.path}
            ...
            <button title={`${ui('Download', '下载')} ${entry.name}`} aria-label={`${ui('Download', '下载')} ${entry.name}`} disabled={busy} onClick={() => props.onDownload(entry)}
